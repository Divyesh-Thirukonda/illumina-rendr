...
void render(const Scene& scene, const string& outputFilename) {
    ...
    cout << "Rendered image to " << outputFilename << endl;
    outFile.close();
}
...
Scene parseScene(const string& filename) {
    ...
    // new mtl colored opacity att
    double alphaR = 0;
    double alphaG = 0;
    double alphaB = 0;
    bool hasAlphaAt = false;
    ...
    scene.bvh.push_back(Vec2(-10000, 10000));
    scene.bvh.push_back(Vec2(-10000, 10000));
    scene.bvh.push_back(Vec2(-10000, 10000));

    while (file >> word) {
        // parse each part of the input file and store it respectively
        if (word == "eye") {
            file >> scene.eye.x >> scene.eye.y >> scene.eye.z;
        } else if (word == "viewdir") {
            file >> scene.viewDir.x >> scene.viewDir.y >> scene.viewDir.z;
        } else if (word == "updir") {
            file >> scene.upDir.x >> scene.upDir.y >> scene.upDir.z;
        } else if (word == "vfov") {
            file >> scene.vfov;
        } else if (word == "imsize") {
            file >> scene.width >> scene.height;
        } else if (word == "bkgcolor") {
            file >> scene.bgColor.x >> scene.bgColor.y >> scene.bgColor.z >> scene.bgEta;
        } else if (word == "mtlcolor") {
            file    >> diffuseColor.x >> diffuseColor.y >> diffuseColor.z
                    >> specularColor.x >> specularColor.y >> specularColor.z
                    >> colorIntensity.x >> colorIntensity.y >> colorIntensity.z
                    >> specIntensity >> alpha >> eta;
            alphaR = 0;
            alphaG = 0;
            alphaB = 0;
            hasAlphaAt = false;
            ...
            double cx, cy, cz, w, i;
            file >> cx >> cy >> cz >> w >> i;
            scene.lights.push_back(Light{Vec3(cx, cy, cz), w, i});
        } else if (word == "sphere") {
            double cx, cy, cz, r;
            file >> cx >> cy >> cz >> r;
            scene.spheres.push_back(Sphere{Vec3(cx, cy, cz), r, diffuseColor, specularColor, colorIntensity, specIntensity, alpha, eta, texture, hasAlphaAt, alphaR, alphaG, alphaB});
        } else if (word == "depthcueing") {
            scene.hasDc = true;
            file    >> scene.dcCol.x >> scene.dcCol.y >> scene.dcCol.z
                    >> scene.dcAlphaMin >> scene.dcAlphaMax >> scene.dcDistMin >> scene.dcDistMax;
        } else if (word == "attlight") {
            scene.hasAt = true;
            double cx, cy, cz, w, i, fattc1, fattc2, fattc3;
            file >> cx >> cy >> cz >> w >> i >> fattc1 >> fattc2 >> fattc3;
            scene.lights.push_back(Light{Vec3(cx, cy, cz), w, i, fattc1, fattc2, fattc3});
        } else if (word == "parallel") {
            scene.parallel = true;
            file >> scene.frustumHeight;
        } else if (word == "v") {
            double x, y, z;
            file >> x >> y >> z;
            ...
            vector<int> vIndices, nIndices, vtIndices;
            
            string faceVertex;
            while (file >> faceVertex) {
                size_t firstSlash = faceVertex.find('/');
                if (firstSlash == string::npos) {
                    // Handle flat shading (v1/vt1)
                    int vIndex = stoi(faceVertex) - 1;  // 0-based index
                    vIndices.push_back(vIndex);
                    vtIndices.push_back(vIndex);  // Same index for texture coordinates
                } else {
                    size_t secondSlash = faceVertex.find('/', firstSlash + 1);
                    if (secondSlash == string::npos) {
                        // (v1/vt1/vn1)
                        int vIndex = stoi(faceVertex.substr(0, firstSlash)) - 1;
                        int vtIndex = stoi(faceVertex.substr(firstSlash + 1)) - 1;
                        vIndices.push_back(vIndex);
                        vtIndices.push_back(vtIndex);
                    } else {
                        // (v1//vn1)
                        int vIndex = stoi(faceVertex.substr(0, firstSlash)) - 1;
                        int vtIndex = 0; // Default texture index is 0 if no texture part is present
                        if (firstSlash + 1 != secondSlash) {
                            vtIndex = stoi(faceVertex.substr(firstSlash + 1, secondSlash - firstSlash - 1)) - 1;
                        }
                        int nIndex = stoi(faceVertex.substr(secondSlash + 1)) - 1;
                        vIndices.push_back(vIndex);
                        vtIndices.push_back(vtIndex);
            ...
            if (nIndices.empty()) {
                scene.triangles.push_back(Triangle{
                    scene.vertices[vIndices[0]],
                    scene.vertices[vIndices[1]],
                    scene.vertices[vIndices[2]],
                    textureCoords[vtIndices[0]], textureCoords[vtIndices[1]], textureCoords[vtIndices[2]],
                    diffuseColor, specularColor, colorIntensity, specIntensity, alpha, eta, texture,
                    hasAlphaAt, alphaR, alphaG, alphaB
                });
            } else {
                scene.triangles.push_back(Triangle{
                    scene.vertices[vIndices[0]],
                    scene.vertices[vIndices[1]],
                    scene.vertices[vIndices[2]],
                    scene.normals[nIndices[0]],
                    scene.normals[nIndices[1]],
                    scene.normals[nIndices[2]],
                    textureCoords[vtIndices[0]], textureCoords[vtIndices[1]], textureCoords[vtIndices[2]],
                    diffuseColor, specularColor, colorIntensity, specIntensity, alpha, eta, texture,
                    hasAlphaAt, alphaR, alphaG, alphaB
                });
            ...
            double nx, ny, nz;
            file >> nx >> ny >> nz;
            scene.normals.push_back(Vec3(nx, ny, nz));
        } else if (word == "vt") { 
            double u, v;
            file >> u >> v;
            ...
            scene.bvh[0] = Vec2(xMin, xMax);
            scene.bvh[1] = Vec2(yMin, yMax);
            scene.bvh[2] = Vec2(zMin, zMax);
        } else if (word == "mtlcolorplus") {
            file    >> diffuseColor.x >> diffuseColor.y >> diffuseColor.z
                    >> specularColor.x >> specularColor.y >> specularColor.z
                    >> colorIntensity.x >> colorIntensity.y >> colorIntensity.z
                    >> specIntensity >> alphaR >> alphaG >> alphaB >> eta;
                    alpha = (alphaR + alphaB + alphaG) / 3;
            hasAlphaAt = true;
        }
    }
    return scene;
